...
type Update struct {
	UpdateID                int64                        `json:"update_id,omitempty"`                 // The update's unique identifier. Update identifiers start from a certain positive number and increase sequentially. This identifier becomes especially handy if you're using webhooks, since it allows you to ignore repeated updates or to restore the correct update sequence, should they get out of order. If there are no new updates for at least a week, then identifier of the next update will be chosen randomly instead of sequentially.
...
type WebhookInfo struct {
	...
	PendingUpdateCount           int64    `json:"pending_update_count,omitempty"`            // Number of updates awaiting delivery
...
type User struct {
	ID                      int64  `json:"id,omitempty"`                          // Unique identifier for this user or bot. This number may have more than 32 significant bits and some programming languages may have difficulty/silent defects in interpreting it. But it has at most 52 significant bits, so a 64-bit integer or double-precision float type are safe for storing this identifier.
...
type Chat struct {
	ID        int64    `json:"id,omitempty"`         // Unique identifier for this chat. This number may have more than 32 significant bits and some programming languages may have difficulty/silent defects in interpreting it. But it has at most 52 significant bits, so a signed 64-bit integer or double-precision float type are safe for storing this identifier.
	Type      ChatType `json:"type,omitempty"`       // Type of the chat, can be either "private", "group", "supergroup" or "channel"
...
type ChatFullInfo struct {
	...
	LastName                           string                `json:"last_name,omitempty"`                               // Optional. Last name of the other party in a private chat
	IsForum                            bool                  `json:"is_forum,omitempty"`                                // Optional. True, if the supergroup chat is a forum (has topics enabled)
	AccentColorID                      int64                 `json:"accent_color_id,omitempty"`                         // Identifier of the accent color for the chat name and backgrounds of the chat photo, reply header, and link preview. See accent colors for more details.
	MaxReactionCount                   int64                 `json:"max_reaction_count,omitempty"`                      // The maximum number of reactions that can be set on a message in the chat
...
type Message struct {
	MessageID                     int                            `json:"message_id,omitempty"`                        // Unique message identifier inside this chat. In specific instances (e.g., message containing a video sent to a big chat), the server might automatically schedule a message instead of sending it immediately. In such cases, this field will be 0 and the relevant message will be unusable until it is actually sent
	...
	Date                          int64                          `json:"date,omitempty"`                              // Date the message was sent in Unix time. It is always a positive number, representing a valid date.
	...
	Chat                          *Chat                          `json:"chat,omitempty"`                              // Chat the message belongs to
...
type MessageId struct {
	MessageID int `json:"message_id,omitempty"` // Unique message identifier. In specific instances (e.g., message containing a video sent to a big chat), the server might automatically schedule a message instead of sending it immediately. In such cases, this field will be 0 and the relevant message will be unusable until it is actually sent
}
...
type InaccessibleMessage struct {
	Chat      *Chat `json:"chat,omitempty"`       // Chat the message belonged to
	MessageID int   `json:"message_id,omitempty"` // Unique message identifier inside the chat
	Date      int64 `json:"date,omitempty"`       // Always 0. The field can be used to differentiate regular and inaccessible messages.
}
...
type MessageEntity struct {
	Type          MessageEntityType `json:"type,omitempty"`            // Type of the entity. Currently, can be "mention" (@username), "hashtag" (#hashtag or #hashtag@chatusername), "cashtag" ($USD or $USD@chatusername), "bot_command" (/start@jobs_bot), "url" (https://telegram.org), "email" (do-not-reply@telegram.org), "phone_number" (+1-212-555-0123), "bold" (bold text), "italic" (italic text), "underline" (underlined text), "strikethrough" (strikethrough text), "spoiler" (spoiler message), "blockquote" (block quotation), "expandable_blockquote" (collapsed-by-default block quotation), "code" (monowidth string), "pre" (monowidth block), "text_link" (for clickable text URLs), "text_mention" (for users without usernames), "custom_emoji" (for inline custom emoji stickers)
	Offset        int64             `json:"offset,omitempty"`          // Offset in UTF-16 code units to the start of the entity
	Length        int64             `json:"length,omitempty"`          // Length of the entity in UTF-16 code units
...
type TextQuote struct {
	Text     string          `json:"text,omitempty"`      // Text of the quoted part of a message that is replied to by the given message
	...
	Position int64           `json:"position,omitempty"`  // Approximate quote position in the original message in UTF-16 code units as specified by the sender
...
type ExternalReplyInfo struct {
	Origin             MessageOrigin       `json:"origin,omitempty"`               // Origin of the message replied to by the given message
...
type ReplyParameters struct {
	MessageID                int             `json:"message_id,omitempty"`                  // Identifier of the message that will be replied to in the current chat, or in the chat chat_id if it is specified
...
type MessageOriginUser struct {
	Type       string `json:"type,omitempty"`        // Type of the message origin, always "user"
	Date       int64  `json:"date,omitempty"`        // Date the message was sent originally in Unix time
	SenderUser *User  `json:"sender_user,omitempty"` // User that sent the message originally
}
...
type MessageOriginHiddenUser struct {
	Type           string `json:"type,omitempty"`             // Type of the message origin, always "hidden_user"
	Date           int64  `json:"date,omitempty"`             // Date the message was sent originally in Unix time
	SenderUserName string `json:"sender_user_name,omitempty"` // Name of the user that sent the message originally
}
...
type MessageOriginChat struct {
	Type            string `json:"type,omitempty"`             // Type of the message origin, always "chat"
	Date            int64  `json:"date,omitempty"`             // Date the message was sent originally in Unix time
	SenderChat      *Chat  `json:"sender_chat,omitempty"`      // Chat that sent the message originally
...
type MessageOriginChannel struct {
	Type            string `json:"type,omitempty"`             // Type of the message origin, always "channel"
	Date            int64  `json:"date,omitempty"`             // Date the message was sent originally in Unix time
	Chat            *Chat  `json:"chat,omitempty"`             // Channel chat to which the message was originally sent
	MessageID       int    `json:"message_id,omitempty"`       // Unique message identifier inside the chat
...
type PhotoSize struct {
	...
	Width        int64  `json:"width,omitempty"`          // Photo width
	Height       int64  `json:"height,omitempty"`         // Photo height
...
type Animation struct {
	FileID       string     `json:"file_id,omitempty"`        // Identifier for this file, which can be used to download or reuse the file
	FileUniqueID string     `json:"file_unique_id,omitempty"` // Unique identifier for this file, which is supposed to be the same over time and for different bots. Can't be used to download or reuse the file.
	Width        int64      `json:"width,omitempty"`          // Video width as defined by the sender
	Height       int64      `json:"height,omitempty"`         // Video height as defined by the sender
	Duration     int64      `json:"duration,omitempty"`       // Duration of the video in seconds as defined by the sender
...
type Audio struct {
	FileID       string     `json:"file_id,omitempty"`        // Identifier for this file, which can be used to download or reuse the file
	FileUniqueID string     `json:"file_unique_id,omitempty"` // Unique identifier for this file, which is supposed to be the same over time and for different bots. Can't be used to download or reuse the file.
	Duration     int64      `json:"duration,omitempty"`       // Duration of the audio in seconds as defined by the sender
...
type Story struct {
	Chat *Chat `json:"chat,omitempty"` // Chat that posted the story
	ID   int64 `json:"id,omitempty"`   // Unique identifier for the story in the chat
}
...
type Video struct {
	...
	StartTimestamp int64       `json:"start_timestamp,omitempty"` // Optional. Timestamp in seconds from which the video will play in the message
	FileName       string      `json:"file_name,omitempty"`       // Optional. Original filename as defined by the sender
	MimeType       string      `json:"mime_type,omitempty"`       // Optional. MIME type of the file as defined by the sender
	FileSize       int64       `json:"file_size,omitempty"`       // Optional. File size in bytes. It can be bigger than 2^31 and some programming languages may have difficulty/silent defects in interpreting it. But it has at most 52 significant bits, so a signed 64-bit integer or double-precision float type are safe for storing this value.
}
...
type VideoNote struct {
	FileID       string     `json:"file_id,omitempty"`        // Identifier for this file, which can be used to download or reuse the file
	FileUniqueID string     `json:"file_unique_id,omitempty"` // Unique identifier for this file, which is supposed to be the same over time and for different bots. Can't be used to download or reuse the file.
	Length       int64      `json:"length,omitempty"`         // Video width and height (diameter of the video message) as defined by the sender
	Duration     int64      `json:"duration,omitempty"`       // Duration of the video in seconds as defined by the sender
	Thumbnail    *PhotoSize `json:"thumbnail,omitempty"`      // Optional. Video thumbnail
	FileSize     int64      `json:"file_size,omitempty"`      // Optional. File size in bytes
}
...
type PaidMediaInfo struct {
	StarCount int64       `json:"star_count,omitempty"` // The number of Telegram Stars that must be paid to buy access to the media
	PaidMedia []PaidMedia `json:"paid_media,omitempty"` // Information about the paid media
}
...
type PaidMediaPreview struct {
	Type     string `json:"type,omitempty"`     // Type of the paid media, always "preview"
...
type PaidMediaPhoto struct {
	Type  string      `json:"type,omitempty"`  // Type of the paid media, always "photo"
	Photo []PhotoSize `json:"photo,omitempty"` // The photo
}
...
type PaidMediaVideo struct {
	Type  string `json:"type,omitempty"`  // Type of the paid media, always "video"
	Video *Video `json:"video,omitempty"` // The video
}
...
type Dice struct {
	...
	Value int64  `json:"value,omitempty"` // Value of the dice, 1-6 for "🎲", "🎯" and "🎳" base emoji, 1-5 for "🏀" and "⚽" base emoji, 1-64 for "🎰" base emoji
}
...
type PollOption struct {
	...
	VoterCount   int64           `json:"voter_count,omitempty"`   // Number of users that voted for this option
}
...
type PollAnswer struct {
	...
	OptionIds []int64 `json:"option_ids,omitempty"` // 0-based identifiers of chosen answer options. May be empty if the vote was retracted.
}
...
type Poll struct {
	...
	TotalVoterCount       int64           `json:"total_voter_count,omitempty"`       // Total number of users that voted in the poll
...
type Location struct {
	Latitude             float64 `json:"latitude,omitempty"`               // Latitude as defined by the sender
	Longitude            float64 `json:"longitude,omitempty"`              // Longitude as defined by the sender
...
type Venue struct {
	Location        *Location `json:"location,omitempty"`          // Venue location. Can't be a live location
...
type WebAppData struct {
	Data       string `json:"data,omitempty"`        // The data. Be aware that a bad client can send arbitrary data in this field.
	ButtonText string `json:"button_text,omitempty"` // Text of the web_app keyboard button from which the Web App was opened. Be aware that a bad client can send arbitrary data in this field.
}
...
type ProximityAlertTriggered struct {
	...
	Distance int64 `json:"distance,omitempty"` // The distance between the users
}
...
type MessageAutoDeleteTimerChanged struct {
	MessageAutoDeleteTime int64 `json:"message_auto_delete_time,omitempty"` // New auto-delete time for messages in the chat; in seconds
}
...
type ChatBoostAdded struct {
	BoostCount int64 `json:"boost_count,omitempty"` // Number of boosts added by the user
}
...
type BackgroundFillSolid struct {
	Type  string `json:"type,omitempty"`  // Type of the background fill, always "solid"
	Color int64  `json:"color,omitempty"` // The color of the background fill in the RGB24 format
}
...
type BackgroundFillGradient struct {
	Type          string `json:"type,omitempty"`           // Type of the background fill, always "gradient"
	TopColor      int64  `json:"top_color,omitempty"`      // Top color of the gradient in the RGB24 format
	BottomColor   int64  `json:"bottom_color,omitempty"`   // Bottom color of the gradient in the RGB24 format
	RotationAngle int64  `json:"rotation_angle,omitempty"` // Clockwise rotation angle of the background fill in degrees; 0-359
}
...
type BackgroundFillFreeformGradient struct {
	Type   string  `json:"type,omitempty"`   // Type of the background fill, always "freeform_gradient"
	Colors []int64 `json:"colors,omitempty"` // A list of the 3 or 4 base colors that are used to generate the freeform gradient in the RGB24 format
}
...
type BackgroundTypeFill struct {
	Type             string         `json:"type,omitempty"`               // Type of the background, always "fill"
	Fill             BackgroundFill `json:"fill,omitempty"`               // The background fill
	DarkThemeDimming int64          `json:"dark_theme_dimming,omitempty"` // Dimming of the background in dark themes, as a percentage; 0-100
}
...
type BackgroundTypeWallpaper struct {
	Type             string    `json:"type,omitempty"`               // Type of the background, always "wallpaper"
...
type BackgroundTypePattern struct {
	Type       string         `json:"type,omitempty"`        // Type of the background, always "pattern"
	Document   *Document      `json:"document,omitempty"`    // Document with the pattern
	Fill       BackgroundFill `json:"fill,omitempty"`        // The background fill that is combined with the pattern
	Intensity  int64          `json:"intensity,omitempty"`   // Intensity of the pattern when it is shown above the filled background; 0-100
...
type BackgroundTypeChatTheme struct {
	Type      string `json:"type,omitempty"`       // Type of the background, always "chat_theme"
...
type ChatBackground struct {
	Type BackgroundType `json:"type,omitempty"` // Type of the background
}
...
type ForumTopicCreated struct {
	Name              string `json:"name,omitempty"`                 // Name of the topic
	IconColor         int64  `json:"icon_color,omitempty"`           // Color of the topic icon in RGB format
...
type SharedUser struct {
	UserID    int64       `json:"user_id,omitempty"`    // Identifier of the shared user. This number may have more than 32 significant bits and some programming languages may have difficulty/silent defects in interpreting it. But it has at most 52 significant bits, so 64-bit integers or double-precision float types are safe for storing these identifiers. The bot may not have access to the user and could be unable to use this identifier, unless the user is already known to the bot by some other means.
...
type UsersShared struct {
	RequestID int64        `json:"request_id,omitempty"` // Identifier of the request
	Users     []SharedUser `json:"users,omitempty"`      // Information about users shared with the bot.
}
...
type ChatShared struct {
	RequestID int64       `json:"request_id,omitempty"` // Identifier of the request
	ChatID    int64       `json:"chat_id,omitempty"`    // Identifier of the shared chat. This number may have more than 32 significant bits and some programming languages may have difficulty/silent defects in interpreting it. But it has at most 52 significant bits, so a 64-bit integer or double-precision float type are safe for storing this identifier. The bot may not have access to the chat and could be unable to use this identifier, unless the chat is already known to the bot by some other means.
...
type VideoChatScheduled struct {
	StartDate int64 `json:"start_date,omitempty"` // Point in time (Unix timestamp) when the video chat is supposed to be started by a chat administrator
}
...
type VideoChatEnded struct {
	Duration int64 `json:"duration,omitempty"` // Video chat duration in seconds
}
...
type VideoChatParticipantsInvited struct {
	Users []User `json:"users,omitempty"` // New members that were invited to the video chat
}
...
type Giveaway struct {
	Chats                         []Chat   `json:"chats,omitempty"`                            // The list of chats which the user must join to participate in the giveaway
	WinnersSelectionDate          int64    `json:"winners_selection_date,omitempty"`           // Point in time (Unix timestamp) when winners of the giveaway will be selected
	WinnerCount                   int64    `json:"winner_count,omitempty"`                     // The number of users which are supposed to be selected as winners of the giveaway
...
type GiveawayWinners struct {
	Chat                          *Chat  `json:"chat,omitempty"`                             // The chat that created the giveaway
	GiveawayMessageID             int64  `json:"giveaway_message_id,omitempty"`              // Identifier of the message with the giveaway in the chat
	WinnersSelectionDate          int64  `json:"winners_selection_date,omitempty"`           // Point in time (Unix timestamp) when winners of the giveaway were selected
	WinnerCount                   int64  `json:"winner_count,omitempty"`                     // Total number of winners in the giveaway
	Winners                       []User `json:"winners,omitempty"`                          // List of up to 100 winners of the giveaway
...
type GiveawayCompleted struct {
	WinnerCount         int64    `json:"winner_count,omitempty"`          // Number of winners in the giveaway
	UnclaimedPrizeCount int64    `json:"unclaimed_prize_count,omitempty"` // Optional. Number of undistributed prizes
...
type UserProfilePhotos struct {
	TotalCount int64         `json:"total_count,omitempty"` // Total number of profile pictures the target user has
...
type WebAppInfo struct {
	Url string `json:"url,omitempty"` // An HTTPS URL of a Web App to be opened with additional data as specified in Initializing Web Apps
}
...
type KeyboardButtonRequestUsers struct {
	RequestID       int64 `json:"request_id,omitempty"`       // Signed 32-bit identifier of the request that will be received back in the UsersShared object. Must be unique within the message
...
type KeyboardButtonRequestChat struct {
	RequestID               int64                    `json:"request_id,omitempty"`                // Signed 32-bit identifier of the request, which will be received back in the ChatShared object. Must be unique within the message
...
type CopyTextButton struct {
	Text string `json:"text,omitempty"` // The text to be copied to the clipboard; 1-256 characters
}
...
type ChatInviteLink struct {
	...
	CreatesJoinRequest      bool   `json:"creates_join_request,omitempty"`       // True, if users joining the chat via the link need to be approved by chat administrators
...
type ChatMemberUpdated struct {
	Chat                    *Chat           `json:"chat,omitempty"`                        // Chat the user belongs to
	From                    *User           `json:"from,omitempty"`                        // Performer of the action, which resulted in the change
	Date                    int64           `json:"date,omitempty"`                        // Date the change was done in Unix time
	OldChatMember           ChatMember      `json:"old_chat_member,omitempty"`             // Previous information about the chat member
	NewChatMember           ChatMember      `json:"new_chat_member,omitempty"`             // New information about the chat member
...
type ChatMemberOwner struct {
	Status      string `json:"status,omitempty"`       // The member's status in the chat, always "creator"
	User        *User  `json:"user,omitempty"`         // Information about the user
	IsAnonymous bool   `json:"is_anonymous,omitempty"` // True, if the user's presence in the chat is hidden
...
type ChatMemberAdministrator struct {
	Status              string `json:"status,omitempty"`                 // The member's status in the chat, always "administrator"
	User                *User  `json:"user,omitempty"`                   // Information about the user
...
type ChatMemberMember struct {
	Status    string `json:"status,omitempty"`     // The member's status in the chat, always "member"
	User      *User  `json:"user,omitempty"`       // Information about the user
...
type ChatMemberRestricted struct {
	Status                string `json:"status,omitempty"`                    // The member's status in the chat, always "restricted"
	User                  *User  `json:"user,omitempty"`                      // Information about the user
	IsMember              bool   `json:"is_member,omitempty"`                 // True, if the user is a member of the chat at the moment of the request
	CanSendMessages       bool   `json:"can_send_messages,omitempty"`         // True, if the user is allowed to send text messages, contacts, giveaways, giveaway winners, invoices, locations and venues
	...
	CanChangeInfo         bool   `json:"can_change_info,omitempty"`           // True, if the user is allowed to change the chat title, photo and other settings
	CanInviteUsers        bool   `json:"can_invite_users,omitempty"`          // True, if the user is allowed to invite new users to the chat
	CanPinMessages        bool   `json:"can_pin_messages,omitempty"`          // True, if the user is allowed to pin messages
	CanManageTopics       bool   `json:"can_manage_topics,omitempty"`         // True, if the user is allowed to create forum topics
	UntilDate             int64  `json:"until_date,omitempty"`                // Date when restrictions will be lifted for this user; Unix time. If 0, then the user is restricted forever
}
...
type ChatMemberLeft struct {
	Status string `json:"status,omitempty"` // The member's status in the chat, always "left"
	User   *User  `json:"user,omitempty"`   // Information about the user
}
...
type ChatMemberBanned struct {
	Status    string `json:"status,omitempty"`     // The member's status in the chat, always "kicked"
	User      *User  `json:"user,omitempty"`       // Information about the user
	UntilDate int64  `json:"until_date,omitempty"` // Date when restrictions will be lifted for this user; Unix time. If 0, then the user is banned forever
}
...
type ChatJoinRequest struct {
	Chat       *Chat           `json:"chat,omitempty"`         // Chat to which the request was sent
	From       *User           `json:"from,omitempty"`         // User that sent the join request
	UserChatID int64           `json:"user_chat_id,omitempty"` // Identifier of a private chat with the user who sent the join request. This number may have more than 32 significant bits and some programming languages may have difficulty/silent defects in interpreting it. But it has at most 52 significant bits, so a 64-bit integer or double-precision float type are safe for storing this identifier. The bot can use this identifier for 5 minutes to send messages until the join request is processed, assuming no other administrator contacted the user.
	Date       int64           `json:"date,omitempty"`         // Date the request was sent in Unix time
...
type Birthdate struct {
	Day   int64 `json:"day,omitempty"`   // Day of the user's birth; 1-31
	Month int64 `json:"month,omitempty"` // Month of the user's birth; 1-12
...
type BusinessLocation struct {
	Address  string    `json:"address,omitempty"`  // Address of the business
...
type BusinessOpeningHoursInterval struct {
	OpeningMinute int64 `json:"opening_minute,omitempty"` // The minute's sequence number in a week, starting on Monday, marking the start of the time interval during which the business is open; 0 - 7 * 24 * 60
	ClosingMinute int64 `json:"closing_minute,omitempty"` // The minute's sequence number in a week, starting on Monday, marking the end of the time interval during which the business is open; 0 - 8 * 24 * 60
}
...
type BusinessOpeningHours struct {
	TimeZoneName string                         `json:"time_zone_name,omitempty"` // Unique name of the time zone for which the opening hours are defined
	OpeningHours []BusinessOpeningHoursInterval `json:"opening_hours,omitempty"`  // List of time intervals describing business opening hours
}
...
type ChatLocation struct {
	Location *Location `json:"location,omitempty"` // The location to which the supergroup is connected. Can't be a live location.
	Address  string    `json:"address,omitempty"`  // Location address; 1-64 characters, as defined by the chat owner
}
...
type ReactionTypeEmoji struct {
	Type  string `json:"type,omitempty"`  // Type of the reaction, always "emoji"
	Emoji string `json:"emoji,omitempty"` // Reaction emoji. Currently, it can be one of "👍", "👎", "❤", "🔥", "🥰", "👏", "😁", "🤔", "🤯", "😱", "🤬", "😢", "🎉", "🤩", "🤮", "💩", "🙏", "👌", "🕊", "🤡", "🥱", "🥴", "😍", "🐳", "❤‍🔥", "🌚", "🌭", "💯", "🤣", "⚡", "🍌", "🏆", "💔", "🤨", "😐", "🍓", "🍾", "💋", "🖕", "😈", "😴", "😭", "🤓", "👻", "👨‍💻", "👀", "🎃", "🙈", "😇", "😨", "🤝", "✍", "🤗", "🫡", "🎅", "🎄", "☃", "💅", "🤪", "🗿", "🆒", "💘", "🙉", "🦄", "😘", "💊", "🙊", "😎", "👾", "🤷‍♂", "🤷", "🤷‍♀", "😡"
}
...
type ReactionTypeCustomEmoji struct {
	Type          string `json:"type,omitempty"`            // Type of the reaction, always "custom_emoji"
	CustomEmojiID string `json:"custom_emoji_id,omitempty"` // Custom emoji identifier
}
...
type ReactionTypePaid struct {
	Type string `json:"type,omitempty"` // Type of the reaction, always "paid"
}
...
type ReactionCount struct {
	Type       ReactionType `json:"type,omitempty"`        // Type of the reaction
	TotalCount int64        `json:"total_count,omitempty"` // Number of times the reaction was added
}
...
type MessageReactionUpdated struct {
	Chat        *Chat          `json:"chat,omitempty"`         // The chat containing the message the user reacted to
	MessageID   int            `json:"message_id,omitempty"`   // Unique identifier of the message inside the chat
	...
	Date        int64          `json:"date,omitempty"`         // Date of the change in Unix time
	OldReaction []ReactionType `json:"old_reaction,omitempty"` // Previous list of reaction types that were set by the user
	NewReaction []ReactionType `json:"new_reaction,omitempty"` // New list of reaction types that have been set by the user
}
...
type MessageReactionCountUpdated struct {
	Chat      *Chat           `json:"chat,omitempty"`       // The chat containing the message
	MessageID int             `json:"message_id,omitempty"` // Unique message identifier inside the chat
	Date      int64           `json:"date,omitempty"`       // Date of the change in Unix time
	Reactions []ReactionCount `json:"reactions,omitempty"`  // List of reactions that are present on the message
}
...
type ForumTopic struct {
	MessageThreadID   int64  `json:"message_thread_id,omitempty"`    // Unique identifier of the forum topic
	Name              string `json:"name,omitempty"`                 // Name of the topic
	IconColor         int64  `json:"icon_color,omitempty"`           // Color of the topic icon in RGB format
	IconCustomEmojiID string `json:"icon_custom_emoji_id,omitempty"` // Optional. Unique identifier of the custom emoji shown as the topic icon
}
...
type BotCommand struct {
	...
	Description string `json:"description,omitempty"` // Description of the command; 1-256 characters.
}
...
type BotCommandScopeDefault struct {
	Type string `json:"type,omitempty"` // Scope type, must be default
}
...
type BotCommandScopeAllPrivateChats struct {
	Type string `json:"type,omitempty"` // Scope type, must be all_private_chats
}
...
type BotCommandScopeAllGroupChats struct {
	Type string `json:"type,omitempty"` // Scope type, must be all_group_chats
}
...
type BotCommandScopeAllChatAdministrators struct {
	Type string `json:"type,omitempty"` // Scope type, must be all_chat_administrators
}
...
type BotCommandScopeChat struct {
	Type   string `json:"type,omitempty"`    // Scope type, must be chat
...
type BotCommandScopeChatAdministrators struct {
	Type   string `json:"type,omitempty"`    // Scope type, must be chat_administrators
	ChatID ChatID `json:"chat_id,omitempty"` // Unique identifier for the target chat or username of the target supergroup (in the format @supergroupusername)
}
...
type BotCommandScopeChatMember struct {
	Type   string `json:"type,omitempty"`    // Scope type, must be chat_member
	ChatID ChatID `json:"chat_id,omitempty"` // Unique identifier for the target chat or username of the target supergroup (in the format @supergroupusername)
...
type BotName struct {
	Name string `json:"name,omitempty"` // The bot's name
}
...
type BotDescription struct {
	Description string `json:"description,omitempty"` // The bot's description
}
...
type BotShortDescription struct {
	ShortDescription string `json:"short_description,omitempty"` // The bot's short description
}
...
type MenuButtonCommands struct {
	Type string `json:"type,omitempty"` // Type of the button, must be commands
}
...
type MenuButtonWebApp struct {
	Type   string      `json:"type,omitempty"`    // Type of the button, must be web_app
	Text   string      `json:"text,omitempty"`    // Text on the button
	WebApp *WebAppInfo `json:"web_app,omitempty"` // Description of the Web App that will be launched when the user presses the button. The Web App will be able to send an arbitrary message on behalf of the user using the method answerWebAppQuery. Alternatively, a t.me link to a Web App of the bot can be specified in the object instead of the Web App's URL, in which case the Web App will be opened as if the user pressed the link.
}
...
type MenuButtonDefault struct {
	Type string `json:"type,omitempty"` // Type of the button, must be default
}
...
type ChatBoostSourcePremium struct {
	Source string `json:"source,omitempty"` // Source of the boost, always "premium"
	User   *User  `json:"user,omitempty"`   // User that boosted the chat
}
...
type ChatBoostSourceGiftCode struct {
	Source string `json:"source,omitempty"` // Source of the boost, always "gift_code"
	User   *User  `json:"user,omitempty"`   // User for which the gift code was created
}
...
type ChatBoostSourceGiveaway struct {
	Source            string `json:"source,omitempty"`              // Source of the boost, always "giveaway"
	GiveawayMessageID int64  `json:"giveaway_message_id,omitempty"` // Identifier of a message in the chat with the giveaway; the message could have been deleted already. May be 0 if the message isn't sent yet.
...
type ChatBoost struct {
	BoostID        string          `json:"boost_id,omitempty"`        // Unique identifier of the boost
	AddDate        int64           `json:"add_date,omitempty"`        // Point in time (Unix timestamp) when the chat was boosted
	ExpirationDate int64           `json:"expiration_date,omitempty"` // Point in time (Unix timestamp) when the boost will automatically expire, unless the booster's Telegram Premium subscription is prolonged
	Source         ChatBoostSource `json:"source,omitempty"`          // Source of the added boost
}
...
type ChatBoostUpdated struct {
	Chat  *Chat      `json:"chat,omitempty"`  // Chat which was boosted
	Boost *ChatBoost `json:"boost,omitempty"` // Information about the chat boost
}
...
type ChatBoostRemoved struct {
	Chat       *Chat           `json:"chat,omitempty"`        // Chat which was boosted
	BoostID    string          `json:"boost_id,omitempty"`    // Unique identifier of the boost
	RemoveDate int64           `json:"remove_date,omitempty"` // Point in time (Unix timestamp) when the boost was removed
	Source     ChatBoostSource `json:"source,omitempty"`      // Source of the removed boost
}
...
type UserChatBoosts struct {
	Boosts []ChatBoost `json:"boosts,omitempty"` // The list of boosts added to the chat by the user
}
...
type BusinessConnection struct {
	ID         string `json:"id,omitempty"`           // Unique identifier of the business connection
	User       *User  `json:"user,omitempty"`         // Business account user that created the business connection
	UserChatID int64  `json:"user_chat_id,omitempty"` // Identifier of a private chat with the user who created the business connection. This number may have more than 32 significant bits and some programming languages may have difficulty/silent defects in interpreting it. But it has at most 52 significant bits, so a 64-bit integer or double-precision float type are safe for storing this identifier.
	Date       int64  `json:"date,omitempty"`         // Date the connection was established in Unix time
	CanReply   bool   `json:"can_reply,omitempty"`    // True, if the bot can act on behalf of the business account in chats that were active in the last 24 hours
	IsEnabled  bool   `json:"is_enabled,omitempty"`   // True, if the connection is active
}
...
type BusinessMessagesDeleted struct {
	...
	Chat                 *Chat   `json:"chat,omitempty"`                   // Information about a chat in the business account. The bot may not have access to the chat or the corresponding user.
	MessageIds           []int64 `json:"message_ids,omitempty"`            // The list of identifiers of deleted messages in the chat of the business account
}
...
type InputMediaVideo struct {
	...
	StartTimestamp        int64           `json:"start_timestamp,omitempty"`          // Optional. Start timestamp for the video in the message
...
type InputPaidMediaPhoto struct {
	Type  string `json:"type,omitempty"`  // Type of the media, must be photo
	Media string `json:"media,omitempty"` // File to send. Pass a file_id to send a file that exists on the Telegram servers (recommended), pass an HTTP URL for Telegram to get a file from the Internet, or pass "attach://<file_attach_name>" to upload a new one using multipart/form-data under <file_attach_name> name. More information on Sending Files: https://core.telegram.org/bots/api#sending-files
}
...
type InputPaidMediaVideo struct {
	Type              string `json:"type,omitempty"`               // Type of the media, must be video
	Media             string `json:"media,omitempty"`              // File to send. Pass a file_id to send a file that exists on the Telegram servers (recommended), pass an HTTP URL for Telegram to get a file from the Internet, or pass "attach://<file_attach_name>" to upload a new one using multipart/form-data under <file_attach_name> name. More information on Sending Files: https://core.telegram.org/bots/api#sending-files
	Thumbnail         string `json:"thumbnail,omitempty"`          // Optional. Thumbnail of the file sent; can be ignored if thumbnail generation for the file is supported server-side. The thumbnail should be in JPEG format and less than 200 kB in size. A thumbnail's width and height should not exceed 320. Ignored if the file is not uploaded using multipart/form-data. Thumbnails can't be reused and can be only uploaded as a new file, so you can pass "attach://<file_attach_name>" if the thumbnail was uploaded using multipart/form-data under <file_attach_name>. More information on Sending Files: https://core.telegram.org/bots/api#sending-files
	Cover             string `json:"cover,omitempty"`              // Optional. Cover for the video in the message. Pass a file_id to send a file that exists on the Telegram servers (recommended), pass an HTTP URL for Telegram to get a file from the Internet, or pass "attach://<file_attach_name>" to upload a new one using multipart/form-data under <file_attach_name> name. More information on Sending Files: https://core.telegram.org/bots/api#sending-files
	StartTimestamp    int64  `json:"start_timestamp,omitempty"`    // Optional. Start timestamp for the video in the message
...
type Sticker struct {
	FileID           string        `json:"file_id,omitempty"`           // Identifier for this file, which can be used to download or reuse the file
	FileUniqueID     string        `json:"file_unique_id,omitempty"`    // Unique identifier for this file, which is supposed to be the same over time and for different bots. Can't be used to download or reuse the file.
	Type             string        `json:"type,omitempty"`              // Type of the sticker, currently one of "regular", "mask", "custom_emoji". The type of the sticker is independent from its format, which is determined by the fields is_animated and is_video.
	Width            int64         `json:"width,omitempty"`             // Sticker width
	Height           int64         `json:"height,omitempty"`            // Sticker height
...
type StickerSet struct {
	...
	StickerType string     `json:"sticker_type,omitempty"` // Type of stickers in the set, currently one of "regular", "mask", "custom_emoji"
...
type InputSticker struct {
	...
	Format       string        `json:"format,omitempty"`        // Format of the added sticker, must be one of "static" for a .WEBP or .PNG image, "animated" for a .TGS animation, "video" for a .WEBM video
	EmojiList    []string      `json:"emoji_list,omitempty"`    // List of 1-20 emoji associated with the sticker
...
type Gift struct {
	ID               string   `json:"id,omitempty"`                 // Unique identifier of the gift
	Sticker          *Sticker `json:"sticker,omitempty"`            // The sticker that represents the gift
	StarCount        int64    `json:"star_count,omitempty"`         // The number of Telegram Stars that must be paid to send the sticker
	UpgradeStarCount int64    `json:"upgrade_star_count,omitempty"` // Optional. The number of Telegram Stars that must be paid to upgrade the gift to a unique one
...
type Gifts struct {
	Gifts []Gift `json:"gifts,omitempty"` // The list of gifts
}
...
type InlineQueryResultPhoto struct {
	...
	ThumbnailUrl          string                `json:"thumbnail_url,omitempty"`            // URL of the thumbnail for the photo
...
type InlineQueryResultGif struct {
	...
	ID                    string                `json:"id,omitempty"`                       // Unique identifier for this result, 1-64 bytes
	GifUrl                string                `json:"gif_url,omitempty"`                  // A valid URL for the GIF file
	...
	ThumbnailUrl          string                `json:"thumbnail_url,omitempty"`            // URL of the static (JPEG or GIF) or animated (MPEG4) thumbnail for the result
	...
	ParseMode             string                `json:"parse_mode,omitempty"`               // Optional. Mode for parsing entities in the caption. See formatting options for more details.
	CaptionEntities       []MessageEntity       `json:"caption_entities,omitempty"`         // Optional. List of special entities that appear in the caption, which can be specified instead of parse_mode
	ShowCaptionAboveMedia bool                  `json:"show_caption_above_media,omitempty"` // Optional. Pass True, if the caption must be shown above the message media
	ReplyMarkup           *InlineKeyboardMarkup `json:"reply_markup,omitempty"`             // Optional. Inline keyboard attached to the message
...
type InlineQueryResultMpeg4Gif struct {
	...
	ID                    string                `json:"id,omitempty"`                       // Unique identifier for this result, 1-64 bytes
	Mpeg4Url              string                `json:"mpeg4_url,omitempty"`                // A valid URL for the MPEG4 file
...
type InlineQueryResultVideo struct {
	...
	MimeType              string                `json:"mime_type,omitempty"`                // MIME type of the content of the video URL, "text/html" or "video/mp4"
	ThumbnailUrl          string                `json:"thumbnail_url,omitempty"`            // URL of the thumbnail (JPEG only) for the video
...
type InlineQueryResultLocation struct {
	...
	ID                   string                `json:"id,omitempty"`                     // Unique identifier for this result, 1-64 Bytes
	Latitude             float64               `json:"latitude,omitempty"`               // Location latitude in degrees
	Longitude            float64               `json:"longitude,omitempty"`              // Location longitude in degrees
...
type InlineQueryResultVenue struct {
	...
	ID                  string                `json:"id,omitempty"`                    // Unique identifier for this result, 1-64 Bytes
	Latitude            float64               `json:"latitude,omitempty"`              // Latitude of the venue location in degrees
	Longitude           float64               `json:"longitude,omitempty"`             // Longitude of the venue location in degrees
...
type InlineQueryResultCachedMpeg4Gif struct {
	Type                  string                `json:"type,omitempty"`                     // Type of the result, must be mpeg4_gif
	ID                    string                `json:"id,omitempty"`                       // Unique identifier for this result, 1-64 bytes
	Mpeg4FileID           string                `json:"mpeg4_file_id,omitempty"`            // A valid file identifier for the MPEG4 file
	Title                 string                `json:"title,omitempty"`                    // Optional. Title for the result
	Caption               string                `json:"caption,omitempty"`                  // Optional. Caption of the MPEG-4 file to be sent, 0-1024 characters after entities parsing
	ParseMode             string                `json:"parse_mode,omitempty"`               // Optional. Mode for parsing entities in the caption. See formatting options for more details.
	CaptionEntities       []MessageEntity       `json:"caption_entities,omitempty"`         // Optional. List of special entities that appear in the caption, which can be specified instead of parse_mode
...
type InputVenueMessageContent struct {
	Latitude        float64 `json:"latitude,omitempty"`          // Latitude of the venue in degrees
	Longitude       float64 `json:"longitude,omitempty"`         // Longitude of the venue in degrees
	Title           string  `json:"title,omitempty"`             // Name of the venue
	Address         string  `json:"address,omitempty"`           // Address of the venue
	FoursquareID    string  `json:"foursquare_id,omitempty"`     // Optional. Foursquare identifier of the venue, if known
	FoursquareType  string  `json:"foursquare_type,omitempty"`   // Optional. Foursquare type of the venue, if known. (For example, "arts_entertainment/default", "arts_entertainment/aquarium" or "food/icecream".)
...
type PreparedInlineMessage struct {
	ID             string `json:"id,omitempty"`              // Unique identifier of the prepared message
	ExpirationDate int64  `json:"expiration_date,omitempty"` // Expiration date of the prepared message, in Unix time. Expired prepared messages can no longer be used
}
...
type LabeledPrice struct {
	...
	Amount int64  `json:"amount,omitempty"` // Price of the product in the smallest units of the currency (integer, not float/double). For example, for a price of US$ 1.45 pass amount = 145. See the exp parameter in currencies.json, it shows the number of digits past the decimal point for each currency (2 for the majority of currencies).
}
...
type Invoice struct {
	...
	Currency       string `json:"currency,omitempty"`        // Three-letter ISO 4217 currency code, or "XTR" for payments in Telegram Stars
	TotalAmount    int64  `json:"total_amount,omitempty"`    // Total price in the smallest units of the currency (integer, not float/double). For example, for a price of US$ 1.45 pass amount = 145. See the exp parameter in currencies.json, it shows the number of digits past the decimal point for each currency (2 for the majority of currencies).
}
...
type ShippingAddress struct {
	CountryCode string `json:"country_code,omitempty"` // Two-letter ISO 3166-1 alpha-2 country code
...
type RefundedPayment struct {
	Currency                string `json:"currency,omitempty"`                   // Three-letter ISO 4217 currency code, or "XTR" for payments in Telegram Stars. Currently, always "XTR"
	TotalAmount             int64  `json:"total_amount,omitempty"`               // Total refunded price in the smallest units of the currency (integer, not float/double). For example, for a price of US$ 1.45, total_amount = 145. See the exp parameter in currencies.json, it shows the number of digits past the decimal point for each currency (2 for the majority of currencies).
	InvoicePayload          string `json:"invoice_payload,omitempty"`            // Bot-specified invoice payload
	TelegramPaymentChargeID string `json:"telegram_payment_charge_id,omitempty"` // Telegram payment identifier
...
type PaidMediaPurchased struct {
	From             *User  `json:"from,omitempty"`               // User who purchased the media
	PaidMediaPayload string `json:"paid_media_payload,omitempty"` // Bot-specified paid media payload
}
...
type RevenueWithdrawalStatePending struct {
	Type string `json:"type,omitempty"` // Type of the state, always "pending"
}
...
type RevenueWithdrawalStateSucceeded struct {
	Type string `json:"type,omitempty"` // Type of the state, always "succeeded"
	Date int64  `json:"date,omitempty"` // Date the withdrawal was completed in Unix time
	Url  string `json:"url,omitempty"`  // An HTTPS URL that can be used to see transaction details
}
...
type RevenueWithdrawalStateFailed struct {
	Type string `json:"type,omitempty"` // Type of the state, always "failed"
}
...
type AffiliateInfo struct {
	...
	CommissionPerMille int64 `json:"commission_per_mille,omitempty"` // The number of Telegram Stars received by the affiliate for each 1000 Telegram Stars received by the bot from referred users
	Amount             int64 `json:"amount,omitempty"`               // Integer amount of Telegram Stars received by the affiliate from the transaction, rounded to 0; can be negative for refunds
...
type TransactionPartnerUser struct {
	Type               string         `json:"type,omitempty"`                // Type of the transaction partner, always "user"
	User               *User          `json:"user,omitempty"`                // Information about the user
...
type TransactionPartnerChat struct {
	Type string `json:"type,omitempty"` // Type of the transaction partner, always "chat"
	Chat *Chat  `json:"chat,omitempty"` // Information about the chat
...
type TransactionPartnerAffiliateProgram struct {
	Type               string `json:"type,omitempty"`                 // Type of the transaction partner, always "affiliate_program"
	SponsorUser        *User  `json:"sponsor_user,omitempty"`         // Optional. Information about the bot that sponsored the affiliate program
	CommissionPerMille int64  `json:"commission_per_mille,omitempty"` // The number of Telegram Stars received by the bot for each 1000 Telegram Stars received by the affiliate program sponsor from referred users
}
...
type TransactionPartnerFragment struct {
	Type            string                 `json:"type,omitempty"`             // Type of the transaction partner, always "fragment"
...
type TransactionPartnerTelegramAds struct {
	Type string `json:"type,omitempty"` // Type of the transaction partner, always "telegram_ads"
}
...
type TransactionPartnerTelegramApi struct {
	Type         string `json:"type,omitempty"`          // Type of the transaction partner, always "telegram_api"
	RequestCount int64  `json:"request_count,omitempty"` // The number of successful requests that exceeded regular limits and were therefore billed
}
...
type TransactionPartnerOther struct {
	Type string `json:"type,omitempty"` // Type of the transaction partner, always "other"
}
...
type StarTransaction struct {
	ID             string             `json:"id,omitempty"`              // Unique identifier of the transaction. Coincides with the identifier of the original transaction for refund transactions. Coincides with SuccessfulPayment.telegram_payment_charge_id for successful incoming payments from users.
	Amount         int64              `json:"amount,omitempty"`          // Integer amount of Telegram Stars transferred by the transaction
	...
	Date           int64              `json:"date,omitempty"`            // Date the transaction was created in Unix time
...
type StarTransactions struct {
	Transactions []StarTransaction `json:"transactions,omitempty"` // The list of transactions
}
...
type PassportFile struct {
	...
	FileDate     int64  `json:"file_date,omitempty"`      // Unix time when the file was uploaded
}
...
type GameHighScore struct {
	Position int64 `json:"position,omitempty"` // Position in high score table for the game
	User     *User `json:"user,omitempty"`     // User
	Score    int64 `json:"score,omitempty"`    // Score
}
